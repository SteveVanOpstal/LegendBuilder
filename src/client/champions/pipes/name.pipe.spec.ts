...
describe('NamePipe', () => {
  ...
  beforeEach(() => { champions = [{name: 'Amumu'}, {name: 'Ahri'}, {name: 'Vel\'Koz'}]; });
  ...
  it('should remove semicolon from names', inject([NamePipe], (pipe) => { expect(pipe.clean('am\'umu')).toBe('amumu'); }));

  it('should lowercase names', inject([NamePipe], (pipe) => { expect(pipe.clean('AmUMu')).toBe('amumu'); }));
});
