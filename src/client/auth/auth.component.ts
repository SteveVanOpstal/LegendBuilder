...
@Component({
  ...
  styles: [require('./auth.css').toString()],
  template: `
    <div class="align-center">
      ...
      <div class="password-repeat"  ngIf="!login">
        <label for="name">Password repeat</label>
        <input type="password" class="form-control" #password placeholder="your password">
      </div>
      <div class="actions">
        <div class="align-center">
          <button type="submit" (click)="auth.login(email.value, password.value)" *ngIf="login">Login</button>
          <button type="submit" (click)="auth.signUp(email.value, password.value)" *ngIf="!login">Sign Up</button>
        </div>
      </div>
