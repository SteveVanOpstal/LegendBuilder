...
import {Item} from '../item' interface Line {enabled: boolean; preview: boolean; name: string; obj: d3.svg.Line<[number, number]>;} @Component({selector: 'graph', changeDetection: ChangeDetectionStrategy.OnPush, directives: [DDragonDirective, AbilitySequenceComponent, NgFor, NgClass], template: `
    <ul class="legend">
    ...
    </svg>`}) export class GraphComponent implements OnChanges, OnInit, AfterContentChecked {@Input() private samples: Samples; @Input() private stats: any; @Input() private champion: any; @Input() private pickedItems: any; private config = config; private svg: any; private xScaleTime = new TimeScale(); private xScaleLevel = new LevelScale(); private yScale = new DataScale(); private xAxisTime = new TimeAxis(); private xAxisLevelLine = new LevelAxisLine(); private xAxisLevelText = new LevelAxisText(); private yAxis = new DataAxis(); private lines = new Array<Line>(); private line: any = d3.svg.line().interpolate('monotone').x((d, i) => {
                     return this.xScaleTime.get()(
                         i * (settings.gameTime /
                              (settings.matchServer.sampleSize - 1)));}).y((d) => { return this.yScale.get()(d);}); constructor(@Inject(ElementRef) private elementRef: ElementRef) {} ngOnInit() {
    this.svg = d3.select(this.elementRef.nativeElement).select('svg');
    this.createAxes();} ngAfterContentChecked() { this.updateLines();} createAxes() {
    ...
    this.svg.select('.y.axis').call(this.yAxis.get());} createLines() {
    this.lines = [];
    ...
    for (let index in this.stats) {
      this.lines.push({
        ...
        preview : false,
        name : index,
        obj : this.line(this.stats[index])
      });
    }} updateLines() {
    ...
    }} clicked(line: Line) { line.enabled = !line.enabled;} mouseEnter(line: Line) {
    if (!line.enabled) {
      line.preview = true;
